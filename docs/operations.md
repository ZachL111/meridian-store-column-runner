...
`meridian-store-column-runner` is centered on databases work in Swift. The technical target is: Develop a Swift command-oriented project for column scenarios with transition tables, invalid-transition tests, and single-node deterministic mode.
